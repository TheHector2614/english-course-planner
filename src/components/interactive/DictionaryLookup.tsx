...
function DictionaryLookupInner() {
  ...
  return (
    <div class="space-y-6">
      ...
      <div class="flex gap-3">
        <div class="relative flex-1">
          <input
            ...
            class="w-full rounded-[var(--radius-lg)] border border-border bg-surface p-4 pr-12 text-base outline-none transition-all focus:border-a2 focus:ring-4 focus:ring-a2-bg/30"
        ...
        <button
          ...
          class="flex items-center gap-2 rounded-[var(--radius-lg)] bg-text px-6 py-4 text-sm font-bold text-surface transition-all hover:opacity-95 disabled:opacity-40 active-scale shadow-md min-h-11"
        >
          {loading ? (
      ...
      {/* Error */}
      {error && (
        <div class="animate-fade-in rounded-[var(--radius-lg)] border border-incorrect p-4 text-sm text-incorrect" style={{ background: "color-mix(in oklch, var(--incorrect) 10%, transparent)" }}>
          {error}
        </div>
      )}
      ...
      {/* Loading skeleton */}
      {loading && (
        <div class="animate-pulse space-y-4 rounded-[var(--radius-lg)] glass-card p-6">
      ...
      {result && !loading && (
        <div class="animate-fade-in-up space-y-6">
          <div class="glass-card rounded-[var(--radius-lg)] p-6 shadow-md">
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div>
                <h2 class="text-3xl sm:text-4xl font-black font-display tracking-tight text-text">{result.word}</h2>
                {phonetics.map((p, i) => (
                  <div key={i} class="mt-2.5 flex items-center gap-2 flex-wrap">
                    {p.text && <span class="text-text-muted font-mono font-medium">{p.text}</span>}
                    {p.audio && (
                      <button
                        onClick={() => playAudio(p.audio!)}
                        class="min-h-11 flex items-center gap-1.5 rounded-full px-5 py-2 text-xs font-bold transition-all hover:shadow-sm active-scale"
                        style={{
              ...
              {/* Save controls */}
              <div class="shrink-0 space-y-2.5 min-w-[170px] w-full sm:w-auto">
                <select
                  value={saveLevel}
                  onChange={(e) => setSaveLevel(e.target.value)}
                  class="w-full rounded-[var(--radius-md)] border border-border bg-surface p-2 text-xs font-semibold outline-none transition-all focus:border-a2 focus:ring-2 focus:ring-a2-bg"
                ...
                <input
                  ...
                  class="w-full rounded-[var(--radius-md)] border border-border bg-surface p-2 text-xs outline-none transition-all focus:border-a2 focus:ring-2 focus:ring-a2-bg"
                  aria-label="Tags"
                />
                <button
                  onClick={searchImages}
                  disabled={imageLoading || !query.trim() || !hasUnsplashKey}
                  class="w-full flex items-center justify-center gap-2 rounded-[var(--radius-md)] bg-surface-alt border border-border px-3 py-2 text-xs font-bold text-text-secondary transition-all hover:bg-border disabled:opacity-40 min-h-11 active-scale"
                >
                  {imageLoading ? (
                    <span class="inline-block h-3 w-3 animate-spin rounded-full border-2 border-text border-t-transparent" />
                  ) : (
                ...
                {!hasUnsplashKey && (
                  <p class="text-[10px] font-semibold text-warning text-center mt-1">
          ...
          {imageResults.length > 0 && (
            <div class="animate-fade-in-up space-y-4">
              <h3 class="text-sm font-bold text-text-secondary">Choose an image to associate with this word</h3>
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {imageResults.map((img, idx) => (
                  <button
                    key={idx}
                    onClick={() => setSelectedImage(img.url)}
                    class={`relative aspect-square rounded-[var(--radius-md)] overflow-hidden border-2 transition-all duration-300 ${
                  ...
                  >
                    <img
                      ...
                      class="w-full h-full object-cover img-outline transition-transform duration-500 hover:scale-105"
                      loading="lazy"
                    />
                    {selectedImage === img.url && (
                    ...
                    <div class="absolute bottom-1 left-1 right-1 text-xs text-white/90 truncate px-2 py-0.5 rounded bg-black/40 backdrop-blur-sm">
                      {img.author}
                    </div>
                  </button>
                ))}
              ...
              {selectedImage && (
                <p class="text-xs text-correct font-semibold text-center">
          ...
          {result.meanings.map((m, mi) => (
            <div key={mi} class="glass-card rounded-[var(--radius-lg)] p-5 transition-all duration-300">
              <span class="inline-block rounded-full px-3 py-1 text-xs font-bold text-a2 bg-a2-bg" style={{ border: "1px solid color-mix(in oklch, var(--a2) 20%, transparent)" }}>
                {m.partOfSpeech}
              </span>
              <div class="mt-4 space-y-4">
                {m.definitions.map((d, di) => (
                  <div key={di} class="animate-fade-in-up border-b border-border-light last:border-b-0 pb-4 last:pb-0" style={{ animationDelay: `${di * 50}ms` }}>
                    <p class="text-base text-text leading-relaxed">{d.definition}</p>
                    {d.example && (
                      <div class="mt-2 flex items-start gap-2 text-sm text-text-secondary italic">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="mt-1 shrink-0">
                          <path d="M10 17l5-5-5-5" />
                        </svg>
                        <span>"{d.example}"</span>
                        <button
                          onClick={() => playExample(d.example!)}
                          class="shrink-0 rounded p-1 text-text-muted transition-colors hover:text-text active-scale"
                    ...
                    {d.synonyms && d.synonyms.length > 0 && (
                      <div class="mt-2.5 flex flex-wrap gap-1.5">
                        {d.synonyms.slice(0, 4).map((s) => (
                          <span key={s} class="rounded-full bg-a1-bg px-2.5 py-0.5 text-xs text-a1 font-semibold" style={{ border: "1px solid color-mix(in oklch, var(--a1) 15%, transparent)" }}>
                            {s}
                          </span>
                        ))}
                      </div>
                    ...
                    <button
                      ...
                      class="mt-3.5 flex items-center gap-2 min-h-11 rounded-[var(--radius-md)] px-5 py-2.5 text-sm font-bold transition-all disabled:opacity-80 active-scale"
                      style={{
